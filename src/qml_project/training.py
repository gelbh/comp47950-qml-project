"""
Simulation training for the [20]-style variational quantum classifier.

Implements:
  - COBYLA-based optimisation (gradient-free, as in [20]).
  - [20]'s shot schedule: 250 → 500 → 750 shots by evaluation number.
...
def train_classifier(
    ...
    *,
    ...
    Follows [20]'s training protocol: gradient-free COBYLA optimisation,
    progressive shot schedule, and random initialisation in
    :math:`[-\\pi, \\pi]`.
    ...
    max_iter : int
        Maximum number of COBYLA iterations (default 200, as in [20]).
    shot_schedule : dict or None
        Mapping ``{eval_number: shots}``.  Default: [20]'s 250/500/750.
    seed : int
        RNG seed for weight initialisation *and* sampler reproducibility.
    test_shots : int
        Shots per sample for test evaluation (default 300, as in [20]).
    sampler
...
def run_multi_seed_experiment(
    ...
    *,
    ...
    This directly addresses QML model volatility ([20], lecture):
    *"Always build/train LOTS of versions of each model.  QML models can
    be quite volatile and sensitive to starting conditions."*
